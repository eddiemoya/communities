...
function category_post_type_endpoints($rules){
    ...
    $newrules = array();
	$newrules['category/(.+?)/(guide|question|post)/?$'] = 'index.php?category_name=$matches[1]&post_type=$matches[2]';
    $newrules['category/(.+?)/(guide|question|post)/page/?([0-9]{1,})/?$'] = 'index.php?category_name=$matches[1]&post_type=$matches[2]&paged=$matches[3]';
